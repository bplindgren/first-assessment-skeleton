  ...
  toString () {
    ...
    switch (true) {
      case (this.command === 'echo'):
        return colors.blue(`${time} <${this.username}> (${this.command}): ${this.contents}`)
      ...
      case (this.command === 'connect') :
        return colors.green(`${time}: <${this.username}> ${this.contents}`)
      ...
      default:
        return colors.magenta(`${time}: (oops...): ${this.contents}`)
    }
  }
